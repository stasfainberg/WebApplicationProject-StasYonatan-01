...
namespace ProbabilityFunctions
{
    public class Classifier
    {
        ...
        public void TrainClassifier(DataTable table)
        {
            ...
            var results = new string[2] { "under", "above" };
            ...
            for (int j = 0; j < results.Length; j++)
            {
                ...
                for (int i = 1; i < table.Columns.Count; i++)
                {
                    row[a] = Helper.Mean(SelectRows(table, i, string.Format("{0} = '{1}'", table.Columns[0].ColumnName, results[j])));
                    row[++a] = Helper.Variance(SelectRows(table, i, string.Format("{0} = '{1}'", table.Columns[0].ColumnName, results[j])));
                    a++;
                }
            }
        }
        ...
        public string Classify(double[] obj)
        {
            ...
            for (int i = 0; i < results.Length; i++)
            {
                ...
                score.Add(results[i], finalScore * 0.5);
            }
